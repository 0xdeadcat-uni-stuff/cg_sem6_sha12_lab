...
void SphereScene<GProgram, VGenerator>::drawVertieces()
{
	...
	glDrawArrays(GL_LINES, this->vGenerator.QuadSize() * this->vGenerator.Count(),
		(this->vGenerator.Size() - this->vGenerator.QuadSize()) * this->vGenerator.Count() - 1);
}
...
inline void SphereScene<GProgram, VGenerator>::initializeScene()
{
	...
	this->program.addMatrix4x4f(glm::lookAt(glm::vec3(0, -1.0f, -2.3f),
		glm::vec3(0, 0, 0), glm::vec3(0, 1, 0)), "view");
}
